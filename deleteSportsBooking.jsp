	...
	<style type="text/css">
		...
		<script type="text/javascript">
			function radioValidate() {
				var radio = document.forms["radioForm"]["yesno"].value;
...
<body>
	...
	<div class="col-12">
	   ...
	   String query1 = "select schedule_ID from schedule where start_time = '" +start_time+ "' AND end_time = '" +end_time+ "'";
	   String query2 = "select trainer_ID from sports_trainer where trainer_name = '" +trainer_name+ "'";
	   
	   ResultSet rs1 = st1.executeQuery(query1);
	   ResultSet rs2 = st2.executeQuery(query2);
	   
	   if (rs1.next())
		   schedule_ID = rs1.getString("schedule_ID");
	   
	   if (rs2.next()) 
		   trainer_ID = rs2.getString("trainer_ID");
	   
	   String query3 = "select member_ID from sports_trainer_booking where sports_trainer_ID = '" +trainer_ID+ "' AND Date = '" +d+ "' AND schedule_ID = '" +schedule_ID+ "'";	  
	   
	   ResultSet rs3 = st3.executeQuery(query3);
	   
	   if (rs3.next())
		   member_ID = rs3.getString("member_ID"); %>
		   
		<form action="DeleteSportsServlet" method="post" name="radioForm" onsubmit="return radioValidate()">
			<table>
				<caption>Trainer Booking Details</caption>
	   			...
	   			<tr>
	   				<td>cancel your booking?</td>
	   				<td><input type="radio" name="yesno" value="yes">Yes</td>
	   				<td><input type="radio" name="yesno" value="no">No</td>
	   				<td><button type="submit" name="yes">delete</button></td>
	   			</tr>
