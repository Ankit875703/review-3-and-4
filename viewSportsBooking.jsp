...
<body>
	...
	<div class="col-12">
		...
		<div class="row">
	      ...
	      String query = "select t.trainer_name,w.sport_name,tb.Date,s.start_time,s.end_time from sports_trainer t,sports w,schedule s,sports_trainer_booking tb where tb.member_ID = '" +id+ "' AND tb.sports_trainer_ID = t.trainer_ID AND tb.schedule_ID = s.schedule_ID AND w.sport_name = (select sport_name from sports_trainer st, sports sp where st.trainer_ID = tb.sports_trainer_ID AND sp.sport_ID = st.sport)";
	   ...
	   <center>
	   		<table>
	   			...
	   			<tr>
	   				...
	   				<td><%=rs.getString("sport_name") %></td>
	   				<td><%=rs.getString("Date") %></td>
	   				<td><%=rs.getString("start_time") %></td>
	   				<td><%=rs.getString("end_time") %></td>
	   				<td><a href="deleteSportsBooking.jsp?trainer_name=<%=rs.getString("trainer_name") %>&Date=<%=rs.getString("Date") %>&start_time=<%=rs.getString("start_time") %>&end_time=<%=rs.getString("end_time") %>&workout_name=<%=rs.getString("sport_name") %>">cancel</a></td>
	   			</tr>
	   			<% } %>
	   		</table>
	   </center>
			...
			<div class="col-md-2 offset-10 <%=bgcolor %> fixed-top" style="height: 100vh">
				...
				<div class="row nav">
					...
					<div class="pl-5">
					<img src="avatar.png" class="img-fluid rounded-circle" style="height: 250px">
				</div>
