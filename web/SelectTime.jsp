...
<head>
	...
	function validate() {
		...
		var time = document.forms["timeanddateForm"]["time"].value;
...
<body>
	...
	<div class="col-12">
	   ...
	   if (Athletic.equalsIgnoreCase("Gym")) {
	   		query1 = "select ts.day from schedule s, trainer t, trainer_schedule ts where t.Trainer_ID = ts.trainer_ID AND ts.schedule_ID = s.schedule_ID AND t.Trainer_ID = '" +trainer_ID+ "'";
	   		query2 = "select s.start_time,s.end_time from schedule s, trainer t, trainer_schedule ts where t.Trainer_ID = ts.trainer_ID AND ts.schedule_ID = s.schedule_ID AND t.Trainer_ID = '" +trainer_ID+ "'";
	   }
	   else if (Athletic.equalsIgnoreCase("Sports")) {
		   query1 = "select ts.day from schedule s, sports_trainer t, sports_trainer_schedule ts where t.Trainer_ID = ts.trainer_ID AND ts.schedule_ID = s.schedule_ID AND t.Trainer_ID = '" +trainer_ID+ "'";
	   	   query2 = "select s.start_time,s.end_time from schedule s, sports_trainer t, sports_trainer_schedule ts where t.Trainer_ID = ts.trainer_ID AND ts.schedule_ID = s.schedule_ID AND t.Trainer_ID = '" +trainer_ID+ "'";
	   }
	   ResultSet rs1 = st1.executeQuery(query1);
	   ResultSet rs2 = st2.executeQuery(query2); %>
	   
	   
	   <form action="GetTimeAndDate" method="post" name="timeanddateForm" onsubmit="return validate()">
		<table>
			<tr>
				<td>Select Date : <% while (rs1.next()) {%>(Available on <%=rs1.getString("day") %>)<% } %></td>
			...
			<tr>
				...
				<td><select name="time">
					...
					<% while (rs2.next()) { %>
					<option><%=rs2.getString("start_time") %>-<%=rs2.getString("end_time") %></option>
					<% } %>
					</select>
				</td>
			</tr>
